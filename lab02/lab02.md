...
При создании продукта можете проинициализировать его иконку каким-то значением
по умолчанию, или оставить пустой, передавая в ответах `null`.
...
пользователей, а сама сеть использует коммутацию пакетов. Найдите
...
Задержкой распространения сигнала пренебречь. Чтобы не возиться с тем, что $X$ может
не делиться нацело на $S$, ответ может быть верен с точностью до небольшой константы.
